...
; if you want you could use the follwong global variables for calculating you new wordlistNEW : ActionListDir, wordlistNEW, ActiveClass, activeTitle
...
; wordlistDir_backupTopOfTempAHL := ActionListDir
...
ActionListDir  := "..\ActionList\SciTEWindow"
...
#Include,.\..\ActionList\activeClassManipulation.inc.ahk


if (!wordlistNEW && false){
    m = ERROR wordlistNEW is EMPTY: `n `n '%wordlistNEW%' = wordlistNEW  `n   17-03-05_14-51 `n `n 'ActionList\wordlistNameFilter.inc.ahk' = wordlistFilterPath  `n (line:%A_LineNumber%) `n %A_ScriptFullPath% = (token293) A_ScriptFullPath   (line: %A_LineNumber%)
   Clipboard := m
...
wordlistNEW := getWordlistNEW173129( activeTitle, ActiveClass, wordlistNEW, ActionListDir )
ToolTipSlowMotion(wordlistNEW, A_LineNumber)
...
getWordlistNEW173129( activeTitle, ActiveClass, wordlistNEW, ActionListDir ) {
...
if ( RegExMatch( activeTitle , "(FreeCommander|Q-Dir \d|Bild ̦ffnen|Anhang speichern|Datei speichern|Speichern unter|ffnen|Dateien/Ordner suchen|Exportieren|Dokument speichern|Select Path|Open File or Project)" ) ) 
      return "..\_globalWordLists\pfade"


SetTitleMatchMode,2 ; 2: A window's title can contain WinTitle anywhere inside it to be a match.

if( RegExMatch( activeTitle  , "i)(\.ahk)" ) || instr( activeTitle , "\TypingAid-master\ActionList\")  ) {
      ...
      sciteAhkGlobal = ..\ActionList\SciTEWindow\_global.ahk
      if(!Fileexist(wl)){
      ...
      FileAppend, _____global generated lib|rr||ahk|run,..\_globalWordListsGenerated\_ahk_global.ahk`n, % wl
...
if(A_ScriptName == "wordlistNameFilter.inc.ahk" ) {  ; thats developer mode. this script is not includet. 08.03.2017 09:14
; Clipboard := wordlistNEW ;  we dont transport usually inside this global file via clipboard 06.03.2017 19:41
 MsgBox,4, %wordlistNEW%, '%wordlistNEW%' = wordlistNEW  `n '%ActionListDir%' = ActionListDir `n  `n %ActiveClass% = ActiveClass `n  17-03-06_18-48 `n `n ( line: %A_LineNumber%), 50
...
if( SubStr( wordlistNEW , -3 ) <> ".ahk" ) ; 06.03.2018 13:09
    wordlistNEW .= ".ahk"
maybeSuperglobalWordList(wordlistNEW, wordlistNEW_time_between , ActiveClass, activeTitle := "" ){
    if( SubStr( wordlistNEW , -3 ) <> ".ahk" ) ; 06.03.2018 13:09
     ...
     if(wordlistNEW <> wordlistNEW_time_between)
          return, % wordlistNEW 
     if(FileExist( "..\ActionList\" . ActiveClass . "\" . wordlistNEW ))
          return, % wordlistNEW 
     ;
     if(!FileExist("..\ActionList\" . ActiveClass . "\_create_own_project.flag"))
          return, % "..\_globalWordListsGenerated\_global.ahk"
    ...
    if(InStr(activeTitle, "token=17-08-10_16-17")){
     ...
     FileDelete, % "..\ActionList\" . ActiveClass . "\_create_own_project.flag"

contend =
(
#Include ..\_globalWordLists\_global.ahk
#Include ..\%ActiveClass%\_global.ahk

; #Include ..\_globalWordLists\examplesForBeginners.ahk
___open global library|rr||ahk|run,..\_globalWordLists\_global.ahk
___open class library|rr||ahk|run,_global.ahk
___open window library |rr||ahk|run,%wordlistNEW%
)
     FileAppend,% "",   % "..\ActionList\" . ActiveClass . "\_global.ahk"
if( FileExist("..\ActionList\" . ActiveClass . "\" . wordlistNEW) ){
    Msgbox,ups wordlistNEW = %wordlistNEW% exist already ==> EXIT `n (%A_LineFile%~%A_LineNumber%) )
    EXIT
}
if( !FileExist("..\ActionList\" . ActiveClass . "\_global.ahk") ){
    Msgbox,ups ==> EXIT `n (%A_LineFile%~%A_LineNumber%) )
    EXIT
}
     FileAppend,% contend,   % "..\ActionList\" . ActiveClass . "\" . wordlistNEW
if( !FileExist("..\ActionList\" . ActiveClass . "\" . wordlistNEW) ){
    Msgbox,ups ==> EXIT `n (%A_LineFile%~%A_LineNumber%) )
EXIT
}
     ...
     run,                    % "..\ActionList\" . ActiveClass . "\" . wordlistNEW
