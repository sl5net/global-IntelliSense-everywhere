...
tests(){
  ...
  if(!tWL || !InStr(tWLbackup,tWL)){ ; tWL is maybe only the file name. thats ok
    Msgbox,:( `ntWLbackup <> tWL `n%tWLbackup% <> %tWL%=tWL `n ( %A_ThisFunc%, %A_LineFile%~%A_LineNumber%) 
    return tWL
  }
  WinActivateTry(tWL,9)
  ...
  if(true){
    ;feedbackMsgBox("try:add ae.. to wordlist now", A_LineNumber . " , " . A_ScriptName,1,1)
    WinActivateTry(tWL,9)
    IfWinNotExist,% tWL
    ...
    Loop,9
    {
      sendAeUeLines(tWLbackup) ; this function do not saved
      Sleep,60
      WinGetActiveTitle, tWLunsaved
      if(tWLunsaved <> tWLbackup)
        break
    }
    if(tWLunsaved <> tWLbackup)
      isLinesAddet := true
    ...
    if(!isLinesAddet)
      Msgbox,:( no lines addet. name of notepaddWindow not chaged: `ntWLbackup == tWLunsaved `n %tWLbackup% == %tWLunsaved% `n ( %A_ThisFunc%, %A_LineFile%~%A_LineNumber%) 
    ;else
    ...
    {
      send2inputBox("uu",1,openInputBoxTitle)
      ControlGetText,inputBoxtext,Edit1,% openInputBoxTitle
...
if(WList>=3) {
  ...
  if(doCloseAllAhk){
    ...
    Loop %WList%
    {
      ...
      g_closedAHKlist[arrayCount] = Title
      WinClose,% Title
    }
  }
}
