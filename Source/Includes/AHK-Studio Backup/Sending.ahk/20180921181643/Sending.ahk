...
wordlistFolderOfThisWordlist := removesSymbolicLinksFromFileAdress(wordlistFolderOfThisWordlist) ; user should could includes direcly from his ahk wordlist, without editing the address 05.03.2018 08:15
...
if(isAHKcode){
    ...
    isDeprecated_OpenA_edit_open_lib := ( isAHKcode && isStartingUnderline && is_OpenA_edit_open_lib && RegExMatch( AHKcode , "^\s*(?:run)\s*,?(.+\.ahk)\s*$\b$" ,  m ))
...
msgbox, % isAHKcode "`=isAHKcode`n`n " lineOfIndex "`n=lineOfIndex`n`n is_OpenA_edit_open_lib=`n" is_OpenA_edit_open_lib `n`n isDeprecated_OpenA_edit_open_lib=`n" isDeprecated_OpenA_edit_open_lib "`n`n" AHKcode
   ...
   if(isDeprecated_OpenA_edit_open_lib || isAHKcode && ( RegExMatch( AHKcode , "^\s*(?:AHK-Studio)\s*,?(.+\.ahk)\s*$\b$" ,  m ) || isDeprecated_OpenA_edit_open_lib ) )
   { ; edit open script
   ; ___global generated open|rr||ahk|run,..\_globalWordListsGenerated\_global.ahk

        m1CorrectedAddress := "..\ActionLists" substr(m1,3)
        runString = AHK-Studio.ahk "%m1CorrectedAddress%"
...
aScriptDir2wordlistFolder := removesSymbolicLinksFromFileAdress(A_ScriptDir "\..\ActionLists") ; user should could includes direcly from his ahk wordlist, without editing the address 05.03.2018 08:15
...
Clipboard := ClipboardBackup
    ...
    Send,{Text}%sending%
       ...
       SendPlay, %sending% ; First do the backspaces, Then send word (Raw because we want the string exactly as in wordlist . ahk)
        ;lll(A_LineNumber, A_LineFile, "%sending% `n >" . sending . "<  `n token=18-03-13_14-44")
        ; Msgbox,sending `n >%sending%< `n (%A_LineFile%~%A_LineNumber%) 
}
...
;>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
      ...
      SendInput, %sending% ; First do the backspaces, Then send word (Raw because we want the string exactly as in wordlist . ahk)
 ...
 if( g_sending_is_buggy )
   ...
   if( g_sending_is_buggy )
      ...
      SendEvent, %sending% ; First do the backspaces, Then send word (Raw because we want the string exactly as in wordlist . ahk)
 ;Msgbox, '%SendValue%' = SendValue  `n `n '%sending%' = sending `n (line:%A_LineNumber%) n
      Return
   }
 ;Msgbox, '%SendValue%' = SendValue  `n `n '%sending%' = sending `n (line:%A_LineNumber%) n
