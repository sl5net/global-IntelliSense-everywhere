...
g_config.sql := { 	select: {  		ignReg: "m)^((?!\bActionListID\b\s*[><=]).)*$" , 		ignIfWhereIsWithoutListID : false, 		showFilePrefix : false  	}, 	template : { 		dir: A_ScriptDir "\sql\template", 		maxNnumberUsedTemplates : 7 	}, 	DB_Browser: A_ProgramFiles " (x86)\DB Browser for SQLite\DB Browser for SQLite.exe" }  
